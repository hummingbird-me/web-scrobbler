...
chrome.runtime.sendMessage({action: 'getScrobbling'}, function (scrobbling) {
    if (typeof scrobbling.chooseData == 'object') {
        scrobbling.chooseData.forEach((element, index) => {
            $('ul').append('<li data-choose="' + index + '"><a href="#">' + element.attributes.canonicalTitle + '</a></li>');
        });
        $('li').click(function() {
            chrome.runtime.sendMessage({action: 'setScrobbling', id: $(this).attr('data-choose')});
